...
async function requestJson<T>(url: string, init: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) throw new RequestError(data.message || "Something went wrong", response.status, data.code, data.needsConfirm);
  return data;
}

class RequestError extends Error {
  constructor(message: string, readonly status: number, readonly code?: string, readonly needsConfirm?: boolean) {
    super(message);
  }
}
        ...
        <aside className={`builder-panel ${mobilePanel ? "builder-panel--open" : ""}`}>
          ...
          <div className="builder-panel-body">
            ...
            {panel === "settings" && <div className="builder-settings">
              ...
              <section><h2>Publishing</h2><p>{state.hasPublished ? "Your edits stay private until you review and publish this version." : "Preview and edit for free. A verified free account is required to own and publish the link. No card required."}</p><p className="builder-trust">Your page does not expire under the current free plan. You get 7 days to edit after first publication.</p></section>
