          ...
          <div className="builder-panel-body">
            ...
            {panel === "edit" && <div className="builder-editor">
              ...
              {editTarget.section === "more" && <fieldset><legend>Private authoring notes</legend>
                {(["workingStyle", "careerDirection", "voiceNotes"] as const).map((field) => <label key={field}><span>{field === "workingStyle" ? "How you work" : field === "careerDirection" ? "What you want to do next" : "How you want to sound"}</span><textarea value={document.privateContext[field] || ""} onChange={(event) => { const value = event.target.value; editLocal((current) => ({ ...current, privateContext: { ...current.privateContext, [field]: value } })); }} onBlur={saveCurrent} /></label>)}
                <label><span>Questions visitors may ask · one “Question | Answer” per line</span><textarea value={document.privateContext.questions.map((item) => `${item.question} | ${item.answer}`).join("\n")} onChange={(event) => { const questions = event.target.value.split("\n").map((line) => { const [question, ...answer] = line.split("|"); return { question: question.trim(), answer: answer.join("|").trim() }; }).filter((item) => item.question && item.answer).slice(0, 12); editLocal((current) => ({ ...current, privateContext: { ...current.privateContext, questions } })); }} onBlur={saveCurrent} /></label>
                <label><span>Concerns you may want to address · one “Concern | Response” per line</span><textarea value={document.privateContext.concerns.map((item) => `${item.concern} | ${item.response}`).join("\n")} onChange={(event) => { const concerns = event.target.value.split("\n").map((line) => { const [concern, ...response] = line.split("|"); return { concern: concern.trim(), response: response.join("|").trim() }; }).filter((item) => item.concern && item.response).slice(0, 12); editLocal((current) => ({ ...current, privateContext: { ...current.privateContext, concerns } })); }} onBlur={saveCurrent} /></label>
                <p className="builder-private-note">These notes guide your private authoring conversation. They are not shown publicly unless you approve wording that uses them.</p>
              </fieldset>}
            </div>}
