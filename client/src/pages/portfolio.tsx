      ...
      <div className="min-h-screen bg-[#17140F] text-[#EDE7DC]" style={{ fontFamily: 'ui-serif, "New York", "Times New Roman", Georgia, serif' }}>
        ...
        {dStats.length > 0 && (
          <div className="px-11 pb-11 border-b border-[#322C22]">
            ...
            <div className="max-w-[460px] border-b border-[#241F17]">
              {dStats.map((stat, i) => (
                <div key={i} className="flex justify-between items-baseline py-4 border-t border-[#241F17]">
                  <span className="text-[28px] font-medium">{stat.value}</span>
                  <span style={{ fontFamily: "-apple-system, sans-serif" }} className="text-[12.5px] text-[#A69C89] text-right">{stat.label}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="px-11 py-12 border-b border-[#322C22] max-w-[680px]" id="mag-qa">
          ...
          <div style={{ fontFamily: "-apple-system, sans-serif" }} className="text-[11.5px] text-[#7A7568] mb-6">Their answers, through an AI trained on their own record.</div>
          <div ref={scrollRef} className="flex flex-col gap-6 mb-6 max-h-[480px] overflow-y-auto">
            {messages.length === 0 && !isStreaming && (
